...
class DespesaCaixa(BaseModel):
    ...
    unidade_id: int = Field(..., description="ID numérico da unidade no WebPosto")
...
class MovimentacaoEspecie(BaseModel):
    ...
    def calcular_diferenca_e_variacao(self) -> Self:
        ...
        # Atualiza os atributos no próprio objeto e retorna self (compatível com __init__)
...
class FechamentoCaixa(BaseModel):
    ...
    def calcular_quebra(self) -> Self:
        quebra = round(self.saldo_esperado_dinheiro - self.saldo_informado_dinheiro, 2)
        object.__setattr__(self, "quebra_caixa", quebra)
        return self
