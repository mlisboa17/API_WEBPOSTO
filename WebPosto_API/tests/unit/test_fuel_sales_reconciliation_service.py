...
class FakeCatalogWithLmcLink:
    """Catálogo em que o produto vendido (20) não tem nome próprio, mas
    compartilha produtoLmcCodigo com outro produto (10) já nomeado."""
        ...
        return WebPostoResponse.ok(
            {
                "products": [
                    {
                        "produtoCodigo": 10,
                        "nomeProduto": "Diesel S10",
                        "grupoCodigo": 24554,
                        "produtoLmcCodigo": 555,
                    },
                    {
                        "produtoCodigo": 20,
                        "nomeProduto": "Produto 20",
                        "grupoCodigo": 24554,
                        "produtoLmcCodigo": 555,
                    },
                ]
...
class FakePaginatorUnnamedProduct:
    async def collect(self, _primary, _fallback, params):
        company = params["empresaCodigo"]
        return WebPostoResponse.ok(
            {
...
async def test_resolves_generic_name_via_lmc_cross_reference() -> None:
    service = FuelSalesReconciliationService(
        object(), catalog=FakeCatalogWithLmcLink(), paginator=FakePaginatorUnnamedProduct()
    )

    response = await service.build("2026-07-01", "2026-07-01", 74014)

    assert response.success is True
    product = response.data["empresas"][0]["produtos"][0]
    assert product["combustivel"] == "Diesel S10"
