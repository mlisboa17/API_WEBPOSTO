...
-- Data: 2026-06-27
-- Versão: 1.1 (sem FK para tenants)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_audit_log_user ON governance.audit_log(user_id);
...
CREATE INDEX IF NOT EXISTS idx_audit_log_status ON governance.audit_log(status);

-- RLS
ALTER TABLE governance.audit_log ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS audit_log_tenant_isolation ON governance.audit_log;
CREATE POLICY audit_log_tenant_isolation ON governance.audit_log
    FOR SELECT
    USING (tenant_id = current_setting('app.current_tenant', true));

DROP POLICY IF EXISTS audit_log_service_insert ON governance.audit_log;
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_approval_tenant ON governance.approval_requests(tenant_id);
CREATE INDEX IF NOT EXISTS idx_approval_status ON governance.approval_requests(status);
CREATE INDEX IF NOT EXISTS idx_approval_requester ON governance.approval_requests(requester_user_id);
CREATE INDEX IF NOT EXISTS idx_approval_created ON governance.approval_requests(created_at DESC);

-- RLS
ALTER TABLE governance.approval_requests ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS approval_requests_tenant_isolation ON governance.approval_requests;
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_security_tenant ON governance.security_events(tenant_id);
CREATE INDEX IF NOT EXISTS idx_security_event_type ON governance.security_events(event_type);
CREATE INDEX IF NOT EXISTS idx_security_severity ON governance.security_events(severity);
CREATE INDEX IF NOT EXISTS idx_security_created ON governance.security_events(created_at DESC);

-- RLS
ALTER TABLE governance.security_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS security_events_tenant_isolation ON governance.security_events;
...
-- ============================================================================
CREATE TABLE IF NOT EXISTS governance.copilot_audit (
    id BIGSERIAL PRIMARY KEY,
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_copilot_tenant ON governance.copilot_audit(tenant_id);
CREATE INDEX IF NOT EXISTS idx_copilot_user ON governance.copilot_audit(user_id);
CREATE INDEX IF NOT EXISTS idx_copilot_session ON governance.copilot_audit(session_id);
CREATE INDEX IF NOT EXISTS idx_copilot_created ON governance.copilot_audit(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_copilot_model ON governance.copilot_audit(model_used);

-- RLS
ALTER TABLE governance.copilot_audit ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS copilot_audit_tenant_isolation ON governance.copilot_audit;
CREATE POLICY copilot_audit_tenant_isolation ON governance.copilot_audit
    FOR SELECT
    USING (tenant_id = current_setting('app.current_tenant', true));

DROP POLICY IF EXISTS copilot_audit_service_insert ON governance.copilot_audit;
...
-- Índices
CREATE INDEX IF NOT EXISTS idx_user_roles_tenant ON governance.user_roles(tenant_id);
CREATE INDEX IF NOT EXISTS idx_user_roles_user ON governance.user_roles(user_id);
CREATE INDEX IF NOT EXISTS idx_user_roles_role ON governance.user_roles(role);

-- RLS
ALTER TABLE governance.user_roles ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS user_roles_tenant_isolation ON governance.user_roles;
...
COMMENT ON TABLE governance.security_events IS 'Eventos de segurança';
COMMENT ON TABLE governance.copilot_audit IS 'Auditoria de uso do Copilot';
