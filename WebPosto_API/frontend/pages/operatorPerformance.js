...
export function renderOperatorPerformance(node, payload, filters, options = {}) {
  ...
  const exportRows = (operators.todos || operators.ranking || []).map((r) => ({
  ...
  renderExecutiveCockpitPage(node, payload, filters, {
    ...
    detailBuilder: (_cockpit, payloadDetail) => {
      const summary = payloadDetail.summary || {};
      const operatorsDetail = payloadDetail.operators || {};
      const pdvs = payloadDetail.pdvs || {};
      const turns = payloadDetail.turns || {};
      const heatmaps = payloadDetail.heatmaps || {};
      const evolution = payloadDetail.evolution || {};
      const intel = payloadDetail.intelligence || {};
      const exec = payloadDetail.executiveAnswers || {};
      ...
      return `
        ...
        <div class="panels-grid">
          ${renderRanking("Top 20 Operadores", operatorsDetail.ranking || operatorsDetail.topMelhores, "funcionarioCodigo", "Op. ")}
    ...
    refreshButtonId: "perfRefresh",
    exportButtonId: "perfExport",
    exportData: exportRows,
    exportFileName: `operator_performance_${filters?.dataFinal || "export"}.csv`,
    defaultView: "operatorPerformance",
