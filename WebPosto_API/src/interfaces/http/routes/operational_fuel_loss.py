    ...
    try:
        ...
        for code in targets:
            ...
            for r in records:
                try:
                    capacidade = float(
                        r.get("capacidade") or r.get("capacidadeLitros") or 0
                    )
                    fisico = float(
                        r.get("volumeAtual")
                    ...
                    entradas = float(
                        r.get("entradas") or r.get("volumeEntradas") or 0
                    )
                    saidas = float(
                        r.get("saidas")
                        or r.get("vendas")
                        or r.get("volumeSaidas")
                        or 0
                    )
                    inicial = float(
                        r.get("estoqueInicial") or r.get("saldoInicial") or 0
                    )
                    # Contábil = Inicial + Entradas − Vendas (quando campos existirem)
                    ...
                    # fallback: volume da predição se tanque veio zerado
                    ...
                    if pred and capacidade <= 0 and pred.capacidade_tanque > 0:
                        ...
                        base_vol = capacidade
                        divergencia_pct = (
                            (abs(divergencia) / base_vol) * 100 if base_vol else 0
                        )
                    ...
                    tanks.append(
                        TankData(
                            id=int(r.get("tanqueCodigo") or r.get("codigo") or 0),
                            name=nome,
                            fuel=prod_nome or (pred.produto_nome if pred else ""),
                            capacity=capacidade,
                            current=volume,
                            temp=temp,
                            ...
                            empresa_codigo=code,
                            empresa_nome=str(
                                r.get("empresaNome") or r.get("empresa") or ""
                            ),
            ...
            for (code, key), p in list(runway_by_key.items()):
                ...
                tanks.append(
                    TankData(
                        ...
                        status=(
                            "CRITICO"
                            if p.status_alerta == "COMPRA_URGENTE"
                            else "DESVIO_SUSPEITO"
                            if p.status_alerta == "ATENCAO"
                            else "NORMAL"
                        ),
                        empresa_codigo=code,
                        autonomia_dias=p.autonomia_dias_restantes,
                        consumo_medio_diario=p.consumo_medio_diario,
                        alerta_autonomia=p.status_alerta,
                        alerta_label=p.alerta_label,
                    )
                )
