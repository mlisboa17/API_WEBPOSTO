"""Snapshot F03.3 — Employee Cash Ledger (TTL 300s, chave employee:ledger)."""
...
EMPLOYEE_LEDGER_SNAPSHOT_TTL_SECONDS = 300.0


class EmployeeLedgerSnapshotService:
    def __init__(
        self,
        overview: NetworkFinancialOverviewService | None = None,
        ledger: EmployeeCashLedgerService | None = None,
        management: ManagementClassificationService | None = None,
        semantic: ExpenseSemanticService | None = None,
        lineage: ExpenseLineageService | None = None,
        output_dir: str = "snapshots/employee_ledger",
        ttl_seconds: float = EMPLOYEE_LEDGER_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        cfg = load_core_config()
        client = WebPostoClient(cfg)
        self._overview = overview or NetworkFinancialOverviewService(client)
        self._ledger = ledger or EmployeeCashLedgerService()
        self._management = management or ManagementClassificationService()
    ...
    def key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"employee:ledger:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
    ...
    async def collect(
        ...
        ledger_payload = await self._ledger.build_ledger_payload(
            self._overview, filters, self._lineage, self._semantic, self._management
        )
        mgmt_payload = await self._management.build_management_payload(
            self._overview, filters, self._lineage, self._semantic
        )
        ...
        master = {
            ...
            "payload": {
                ...
                "management": mgmt_payload,
            },
            "forensics": ledger_payload.get("forensics"),
            "balanceSummary": ledger_payload.get("balanceSummary"),
            "managementSummary": mgmt_payload.get("summary"),
        }
