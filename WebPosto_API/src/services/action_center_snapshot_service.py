...
ACTION_CENTER_TTL = 300.0


class ActionCenterSnapshotService:
    def __init__(
        self,
        service: ActionCenterService | None = None,
        output_dir: str = "snapshots/action_center",
        ttl_seconds: float = ACTION_CENTER_TTL,
    ) -> None:
        self._service = service or ActionCenterService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"action:center:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
