"""Snapshot F04.4 — Management Action Center."""
...
MAC_SNAPSHOT_TTL_SECONDS = 300.0


class ManagementActionCenterSnapshotService:
    def __init__(
        self,
        service: ManagementActionCenterService | None = None,
        output_dir: str = "snapshots/management_action_center",
        ttl_seconds: float = MAC_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._service = service or ManagementActionCenterService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"management:action:center:all:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
