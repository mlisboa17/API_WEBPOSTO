"""Snapshot F04.5 — Goals & Campaign Engine."""
...
GOALS_SNAPSHOT_TTL_SECONDS = 300.0


class GoalsCampaignEngineSnapshotService:
    def __init__(
        self,
        service: GoalsCampaignEngineService | None = None,
        output_dir: str = "snapshots/goals_campaign_engine",
        ttl_seconds: float = GOALS_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._service = service or GoalsCampaignEngineService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"goals:campaign:all:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
