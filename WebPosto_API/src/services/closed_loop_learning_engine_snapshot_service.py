"""Snapshot F05.5 — Closed Loop Learning Engine."""
...
class ClosedLoopLearningEngineSnapshotService:
    def __init__(
        self,
        service: ClosedLoopLearningEngineService | None = None,
        output_dir: str = "snapshots/closed_loop_learning_engine",
        ttl_seconds: float = LEARNING_TTL,
    ) -> None:
        self._service = service or ClosedLoopLearningEngineService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"learning:engine:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
