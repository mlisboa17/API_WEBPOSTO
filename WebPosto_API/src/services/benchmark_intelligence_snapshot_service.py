"""Snapshot F04.6 — Benchmark Intelligence."""
...
BENCHMARK_SNAPSHOT_TTL_SECONDS = 300.0


class BenchmarkIntelligenceSnapshotService:
    def __init__(
        self,
        service: BenchmarkIntelligenceService | None = None,
        output_dir: str = "snapshots/benchmark_intelligence",
        ttl_seconds: float = BENCHMARK_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._service = service or BenchmarkIntelligenceService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"benchmark:all:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
