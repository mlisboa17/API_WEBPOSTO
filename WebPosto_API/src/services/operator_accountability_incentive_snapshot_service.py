"""Snapshot F04.1 — People Intelligence / Accountability & Incentive."""
...
PEOPLE_SNAPSHOT_TTL_SECONDS = 300.0


class OperatorAccountabilityIncentiveSnapshotService:
    def __init__(
        self,
        service: OperatorAccountabilityIncentiveService | None = None,
        output_dir: str = "snapshots/people_intelligence",
        ttl_seconds: float = PEOPLE_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._service = service or OperatorAccountabilityIncentiveService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"operator:people:all:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
