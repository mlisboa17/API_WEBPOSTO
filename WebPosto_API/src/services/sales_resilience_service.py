"""HOTFIX P0 — /v1/sales live-first non-blocking com fallback snapshot."""
...
from src.gateway.sales_circuit import (
    SALES_GATE_ENDPOINT,
    record_sales_live_failure,
    sales_circuit_open,
    sales_circuit_status,
)
...
class SalesResilienceService:
    ...
    def _resilience_meta(
        ...
        *,
        ...
        meta: dict[str, Any] = {
            ...
            "banner": banner,
            "circuitStatus": sales_circuit_status(self._client),
            "salesGate": SALES_GATE_ENDPOINT,
    ...
    async def _attempt_live(
        ...
        *,
        ...
        task = asyncio.create_task(self._overview.get_sales(filters, page=page, limit=limit))
        try:
        ...
        *,
        ...
        return {
            ...
            "data": {
                ...
                "total": 0,
                "data": [],
                "consolidado": {"total_vendas": "0", "qtd_vendas": 0},
                "synthetic": True,
            },
            "error": {
                "type": "SALES_DEGRADED",
    ...
    async def get_sales(
        ...
        snap = self._snapshots.load_kind("financial_sales", key)

        if sales_circuit_open(self._client):
            if snap and snap.get("data"):
                return self._snapshot_response(
                    ...
                    live_error=WebPostoError(
                        endpoint=SALES_GATE_ENDPOINT,
                        type="CIRCUIT_OPEN",
                        message="Circuito sales aberto — usando snapshot",
                    ),
                )
            return self._degraded_response(
                ...
                live_error=WebPostoError(
                    endpoint=SALES_GATE_ENDPOINT,
                    type="CIRCUIT_OPEN",
                    message="Circuito sales aberto",
                ),
            )

        elapsed = monotonic() - started
        remaining = settings.sales_total_budget_seconds - elapsed
        live_timeout = min(settings.sales_live_timeout_seconds, max(0.5, remaining))
        ...
        if live.success and live.data:
            self._snapshots.save_kind("financial_sales", key, live.data, source="live")
            body = live.to_dict()
            body["source"] = "live"
            body["degraded"] = False
        ...
        if reason in {"live_timeout", "live_error"}:
            record_sales_live_failure(self._client, block_seconds=settings.circuit_breaker_timeout)
