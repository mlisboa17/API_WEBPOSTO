...
class FinancialImpact(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"estimated_value": 5000.0, "currency": "BRL", "impact_type": "recoverable"}]})
    ...
    impact_type: str = Field(..., description="Type: loss_prevented, recoverable, gain_opportunity, cost_avoidance")
    probability: float = Field(..., ge=0, le=1, description="Probability of achieving this impact (0-1)")
...
class DecisionSource(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"endpoint": "/v1/financial/overview", "service": "FinancialOverviewService", "data_timestamp": "2026-06-29T10:00:00Z"}]})
    ...
    parameters: Optional[Dict[str, Any]] = Field(default=None, description="Parameters used in the query")


class DecisionAction(BaseModel):
    """A specific action the owner should take."""
    model_config = ConfigDict(json_schema_extra={"examples": [{"id": "act_001", "title": "Review overdue accounts", "description": "3 accounts totaling R$ 12,450 are overdue. Contact customers immediately."}]})
    ...
    # Metadata
    category: str = Field(..., description="Category: revenue, expense, cash, inventory, staff, etc.")
...
class MoneyAtRiskFinding(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"id": "risk_001", "title": "Revenue dropping 25%", "risk_type": "revenue_decline"}]})
    ...
    # Risk specifics
    risk_type: str = Field(..., description="Type: revenue_decline, expense_increase, cash_shortage, margin_drop, etc.")
...
class RecoverableMoneyFinding(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"id": "rec_001", "title": "R$ 8,200 in overdue accounts", "recovery_type": "overdue_receivable"}]})
    ...
    # Recovery specifics
    recovery_type: str = Field(..., description="Type: overdue_receivable, unbilled_sale, duplicate_payment, billing_error, etc.")
    
    # Financial
    recoverable_amount: float = Field(..., description="Amount that can be recovered")
    recovery_probability: float = Field(..., ge=0, le=1, description="Probability of successful recovery")
    recovery_timeframe_days: int = Field(..., description="Expected days to recover")
    
    # Details
    affected_customers: Optional[int] = Field(default=None, description="Number of customers affected")
    affected_transactions: Optional[int] = Field(default=None, description="Number of transactions")
    oldest_overdue_days: Optional[int] = Field(default=None, description="Days since oldest item became due")
...
class GrowthOpportunity(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"id": "opp_001", "title": "Diesel sales up 40%", "opportunity_type": "product_trend"}]})
    ...
    opportunity_type: str = Field(..., description="Type: product_trend, margin_improvement, volume_increase, mix_optimization, etc.")
    ...
    investment_required: Optional[float] = Field(default=None, description="Investment needed to capture")
...
class DailyDecision(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"id": "dec_001", "rank": 1, "title": "Contact 3 overdue customers", "total_score": 95.5}]})
...
class BusinessHealthMetrics(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"score": 78.5, "status": "attention", "trend": "improving"}]})
...
class OwnerActionCenterSummary(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"examples": [{"tenant_id": "tenant_001", "empresa_codigo": "11495", "generated_at": "2026-06-29T10:00:00Z"}]})
    ...
    # Money sections
    money_at_risk: List[MoneyAtRiskFinding] = Field(default_factory=list, description="Active risks")
    money_at_risk_total: float = Field(..., description="Total money at risk")
    
    recoverable_money: List[RecoverableMoneyFinding] = Field(default_factory=list, description="Recoverable funds")
    recoverable_total: float = Field(..., description="Total recoverable amount")
    
    opportunities: List[GrowthOpportunity] = Field(default_factory=list, description="Growth opportunities")
    ...
    # Trust
    confidence_average: float = Field(..., ge=0, le=1, description="Average confidence across all findings")
