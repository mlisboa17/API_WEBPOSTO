...
from .schemas import (
    OwnerActionCenterSummary, BusinessHealthMetrics, DailyDecision,
    MoneyAtRiskFinding, RecoverableMoneyFinding, GrowthOpportunity,
    DecisionSource, ConfidenceLevel, BaselineConfig, RiskThresholds
)
...
class OwnerIntelligenceEngine:
    ...
    def __init__(
        ...
        self.growth_opportunities_engine = growth_opportunities_engine or GrowthOpportunitiesEngine()
    ...
    async def generate_action_center_summary(
        ...
        try:
            ...
            business_health = self._calculate_business_health(
                ...
                recoverable_money,
                opportunities,
                data_sources.financial_overview
            )
            ...
            recoverable_total = sum(r.recoverable_amount * r.recovery_probability for r in recoverable_money)
            ...
            confidence_average = sum(all_confidences) / len(all_confidences) if all_confidences else 0
            
            # 8. Count actions by priority
            critical_actions = sum(1 for d in all_decisions if d.action.priority.value == "critical")
    ...
    def _calculate_business_health(
        ...
        try:
            ...
            total_recoverable = sum(r.recoverable_amount * r.recovery_probability for r in recoverable_money)
            ...
            opportunity_bonus = min(10, (total_opportunity / revenue) * 100 * 10) if revenue > 0 else 0
            ...
            expense_health = max(0, min(100, 80 - sum(1 for f in money_at_risk if "expense" in f.risk_type) * 10))
            cash_health = max(0, min(100, 80 - sum(1 for f in money_at_risk if "cash" in f.risk_type) * 15))
            margin_health = max(0, min(100, 80 - sum(1 for f in money_at_risk if "margin" in f.risk_type) * 10))
            operational_health = max(0, min(100, 70 + recovery_bonus + opportunity_bonus))
            
            # Generate recommendations
            recommendations = []
            if money_at_risk:
                recommendations.append(f"Address {len(money_at_risk)} active risks to improve health")
            if recoverable_money:
                recommendations.append(f"Recover R$ {total_recoverable:,.0f} in outstanding amounts")
            if opportunities:
            ...
            return BusinessHealthMetrics(
                ...
                summary=self._generate_health_summary(score, status, trend, money_at_risk, recoverable_money),
