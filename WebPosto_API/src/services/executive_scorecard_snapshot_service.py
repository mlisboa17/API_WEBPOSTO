"""Snapshot F04.7 — Executive Scorecard."""
...
SCORECARD_SNAPSHOT_TTL_SECONDS = 300.0


class ExecutiveScorecardSnapshotService:
    def __init__(
        self,
        service: ExecutiveScorecardService | None = None,
        output_dir: str = "snapshots/executive_scorecard",
        ttl_seconds: float = SCORECARD_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._service = service or ExecutiveScorecardService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"executive:scorecard:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
