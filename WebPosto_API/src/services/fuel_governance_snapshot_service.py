"""Snapshot F06.5 — Fuel Governance."""
...
class FuelGovernanceSnapshotService:
    def __init__(
        self,
        service: FuelGovernanceService | None = None,
        output_dir: str = "snapshots/fuel_governance",
        ttl_seconds: float = FUEL_GOV_TTL,
    ) -> None:
        self._service = service or FuelGovernanceService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"fuel:gov:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
