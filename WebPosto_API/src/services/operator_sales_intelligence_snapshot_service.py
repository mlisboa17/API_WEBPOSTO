"""Snapshot F04.0 — Operator Sales Intelligence."""
...
INTELLIGENCE_SNAPSHOT_TTL_SECONDS = 300.0


class OperatorSalesIntelligenceSnapshotService:
    def __init__(
        self,
        intelligence: OperatorSalesIntelligenceService | None = None,
        output_dir: str = "snapshots/operator_intelligence",
        ttl_seconds: float = INTELLIGENCE_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._intel = intelligence or OperatorSalesIntelligenceService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"operator:intelligence:all:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
    ...
    async def get_or_collect(
        ...
        resp = await self._intel.build(data_inicial, data_final, empresa_codigo)
        if not resp.success:
