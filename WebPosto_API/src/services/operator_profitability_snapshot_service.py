"""Snapshot F04.2 — Operator Profitability & ROI."""
...
PROFITABILITY_SNAPSHOT_TTL_SECONDS = 300.0


class OperatorProfitabilitySnapshotService:
    def __init__(
        self,
        service: OperatorProfitabilityService | None = None,
        output_dir: str = "snapshots/operator_profitability",
        ttl_seconds: float = PROFITABILITY_SNAPSHOT_TTL_SECONDS,
    ) -> None:
        self._service = service or OperatorProfitabilityService()
    ...
    def master_key(cls, data_inicial: str, data_final: str, empresa_codigo: str | int | None) -> str:
        return f"operator:profitability:all:{cls._suffix(data_inicial, data_final, empresa_codigo)}"
